...
const normalizeIdentityCache = new Map<string, string>();

function normalizeIdentity(value: unknown): string {
    const raw = String(value ?? '');
    const cached = normalizeIdentityCache.get(raw);
    if (cached !== undefined) {
        return cached;
    }
    if (normalizeIdentityCache.size > 10_000) {
        normalizeIdentityCache.clear();
    }
    const normalized = raw.trim().toLowerCase().replace(/[^a-z0-9]/g, '');
    normalizeIdentityCache.set(raw, normalized);
    return normalized;
}

const trimmedEntityNameCache = new Map<string, string>();

function trimEntityName(value: unknown): string {
    const raw = String(value ?? '');
    const cached = trimmedEntityNameCache.get(raw);
    if (cached !== undefined) {
        return cached;
    }
    if (trimmedEntityNameCache.size > 10_000) {
        trimmedEntityNameCache.clear();
    }
    const trimmed = raw.replace(/^,+/, '').trim();
    trimmedEntityNameCache.set(raw, trimmed);
    return trimmed;
}
