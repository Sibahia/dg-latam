...
export class CombatHandler {
    ...
    private static readonly combatLookupKeyCache = new Map<string, string>();

    private static normalizeCombatLookupKey(value: unknown): string {
        const raw = String(value ?? '');
        const cached = CombatHandler.combatLookupKeyCache.get(raw);
        if (cached !== undefined) {
            return cached;
        }
        if (CombatHandler.combatLookupKeyCache.size > 10_000) {
            CombatHandler.combatLookupKeyCache.clear();
        }
        const normalized = raw.toLowerCase().replace(/[^a-z0-9]+/g, '');
        CombatHandler.combatLookupKeyCache.set(raw, normalized);
        return normalized;
    }
