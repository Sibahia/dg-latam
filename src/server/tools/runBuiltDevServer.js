...
// ts-node. Keeps `pnpm run dev:built` pointed at localhost with Mongo off, so it exercises the
// production build without touching live game data.
function startBuiltDevServer() {
    if (!fs.existsSync(distMainPath)) {
        console.error('[DevServer] dist/main.js is missing. Run `pnpm run build` first.');
        process.exitCode = 1;
        return;
    }
