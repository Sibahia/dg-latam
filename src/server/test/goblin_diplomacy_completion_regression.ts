...
function createBoss(id: number, name: string, hp: number): any {
    return {
        ...
        characterName: `,${name}`,
        clientSpawned: true,
        // A terminal boss that reaches the completion system was killed by the
        // player, so it carries verified damage (a scripted dead copy must not
        // satisfy the required boss).
